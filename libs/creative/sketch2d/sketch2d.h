...
// VG
//
struct NVGcontext* getVg();
struct NVGcontext* nvgCreateGL(unsigned flags);
void nvgDeleteGL(struct NVGcontext *);
int sketch2d_main( int argc, const char **argv );
...
void nvgDeleteGL(struct NVGcontext *vg) {
#ifdef NANOVG_GL2_IMPLEMENTATION
    return nvgDeleteGL2( vg );
#else
    return nvgDeleteGL3( vg );
#endif
}
