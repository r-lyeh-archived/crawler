...
void loadexts()
{
    ...
    if(!GLEW_VERSION_2_1) fatal("OpenGL version 2.1 required, found version: %s", version);

    hasUBO = glewGetExtension("GL_ARB_uniform_buffer_object") != 0;
}
...
void renderiqm()
{
    ...
    if(numframes > 0)
    {
        ...
        {
            glUniformMatrix3x4fv(gpuskin.getparam("bonemats"), numjoints, GL_FALSE, outframe[0].a.v);
        }
    }
    else 
    {
...
void initgl()
{
    ...
    if(hasUBO)
    {
        ...
        glGetActiveUniformBlockiv(gpuskin.program, blockidx, GL_UNIFORM_BLOCK_DATA_SIZE, &ubosize);
        glGetActiveUniformsiv(gpuskin.program, 1, &bonematsidx, GL_UNIFORM_OFFSET, &bonematsoffset);
